...
제네릭 가변인수는 컴파일 시점에, 공변 배열은 런타임 시점에 오류를 잡아낼 수 있다. 그러나 제네릭 가변인수 배열로 받으면 Object로 전달되므로  불공변 오류가 발생할 수 있다. 이를테면 제네릭 가변인수 배열로 변환된 `Object[]`를 `String[]`에 할당하려고 한다거나.  `javac -Xlint:unchecked`로 힙오염이 발생 가능한 코드인지도 확인할 수 있다. 또는 `@SuppressedWarnings("unchecked")`하면 관련 오류가 나온다. 제네릭 가변 배열은 외부에 노출하지 말아야하고 신뢰하지도 말아야 한다.

스트림에서는 부작용없는 함수를 사용해야 한다.
